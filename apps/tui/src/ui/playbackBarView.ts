...
export function buildPlaybackBarContent(input: PlaybackBarInput): PlaybackBarContent {
  ...
  const coverOffset = hasCover ? 9 : 0;
  const innerWidth = Math.max(20, input.width - 4 - coverOffset);

  if (input.width < NARROW_BREAKPOINT) {
    const title = truncate(rawTitle, Math.max(10, innerWidth - 10));
    const artist = truncate(rawArtist, Math.max(8, innerWidth - 10));
    return {
      variant: 'narrow',
      line1: `${icon} ${title}`,
      line2: `${pos} / ${dur}`,
  ...
  const line1 = `${left1}${' '.repeat(pad1)}${badges}`;

  // Row 2: Full-width progress bar
  const line2 = progressBar;
  ...
  const line3 = truncate(rawMeta, innerWidth);
  return {
