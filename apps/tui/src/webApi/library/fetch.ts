...
export async function fetchSavedTracks(
  ...
  return {
    ...
    offset: safeOffset,
    limit: safeLimit,
    hasMore: safeOffset + rawItems.length < total,
  };
}
...
export async function fetchFollowedArtists(
  ...
  const nextUrlFrom = (p: FollowedPage) =>
    p.after
      ? `/me/following?type=artist&limit=${safeLimit}&after=${encodeURIComponent(p.after)}`
      : undefined;
...
export async function fetchSavedShows(
  ...
  const json = await transport.request(
    `/me/shows?limit=${safeLimit}&offset=${safeOffset}&market=from_token`,
  );
  const rawItems = toArray(pickObjectKey(json, 'items'));
  const shows: CatalogShowT[] = [];
  for (const item of rawItems) {
    const raw =
      item !== null && typeof item === 'object' && 'show' in (item as Record<string, unknown>)
